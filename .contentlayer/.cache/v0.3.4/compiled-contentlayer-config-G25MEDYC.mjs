...
var renderInline = (text) => {
  let result = text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
...
var markdownToHtml = (markdown) => {
  ...
  lines.forEach((line) => {
    ...
    if (trimmed.startsWith("###### ")) {
      ...
      html.push(
        `<h6 class="mt-4 text-sm font-semibold uppercase tracking-wide text-slate-700">${renderInline(
          text
        )}</h6>`
      );
      return;
    ...
    if (trimmed.startsWith("##### ")) {
      ...
      html.push(`<h5 class="mt-5 text-base font-semibold text-slate-800">${renderInline(text)}</h5>`);
      return;
    }
    if (trimmed.startsWith("#### ")) {
      closeList();
      ...
      html.push(`<h4 class="mt-6 text-lg font-semibold text-slate-900">${renderInline(text)}</h4>`);
      return;
    }
    if (trimmed.startsWith("### ")) {
      closeList();
      ...
      const idBase = slugify(text) || "section";
      const current = slugCounts[idBase] ?? 0;
      slugCounts[idBase] = current + 1;
      const id = current ? `${idBase}-${current}` : idBase;
    ...
    if (imageMatch) {
      ...
      html.push(
        `<figure class="my-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><img src="${src}" alt="${renderInline(
          alt
        )}" loading="lazy" class="w-full object-cover" />${caption ? `<figcaption class="px-4 py-3 text-sm font-semibold text-slate-800">${renderInline(
          caption
        )}</figcaption>` : ""}</figure>`
      );
      return;
    ...
    if (calloutMatch) {
      ...
      if (type === "tip") {
        html.push(
          `<div class="${base} border border-green-200 bg-green-50 text-green-900"><span class="text-xl">\u{1F4A1}</span><div>${text}</div></div>`
        );
      } else if (type === "info") {
        html.push(
...
var ContentPage = defineDocumentType(() => ({
  ...
  computedFields: {
    ...
    html: {
      type: "string",
      resolve: (doc) => markdownToHtml(doc.body.raw).html
    }
  }
}));
...
//# sourceMappingURL=compiled-contentlayer-config-G25MEDYC.mjs.map
